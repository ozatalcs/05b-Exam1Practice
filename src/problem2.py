...
def problem2a(circle, rectangle, window):
    ...
    window.render()
    window.continue_on_mouse_click()
    q = rg.Point(rectangle.get_upper_right_corner().x, rectangle.get_upper_right_corner().y)
    w = rg.Point(rectangle.get_lower_left_corner().x, rectangle.get_lower_left_corner().y)
    line = rg.Line(q, w)
...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n):
        rectangle = rg.Rectangle(rg.Point(rect.get_upper_right_corner().x + k * delta,rect.get_upper_right_corner().y - k*delta), rg.Point(rect.get_lower_left_corner().x - k * delta,rect.get_lower_left_corner().y + k * delta))
